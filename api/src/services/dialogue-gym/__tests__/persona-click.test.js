...
describe('what it refuses to simulate, and says so', () => {
  test('a directory autocomplete: a pick is a real record, and inventing one fabricates data', async () => {
    expect(await clickFor(ctrl('autocomplete'), 'Maria Ivanova'))
      .toEqual({ controlAction: null, why: 'directory_needs_a_real_pick' });
  });

  test('declining a confirm means searching the directory — also un-simulable', async () => {
    expect(await clickFor(ctrl('confirm'), 'No, it is for a colleague'))
  ...
  test('an unclear yes stays free text — the model can ask again', async () => {
...
describe('the parsers, on their own', () => {
  test('dateFrom prefers an explicit ISO date to a loose one', async () => {
    expect(dateFrom('between 2026-09-01 and 30/09/2026')).toBe('2026-09-01');
  });

  test('dateFrom reads a loose date day-first, as every persona here writes it', async () => {
    expect(dateFrom('01/10/2026')).toBe('2026-10-01');
  });

  test('numberFrom survives thousands separators and decimals', async () => {
    expect(numberFrom('12 months')).toBe(12);
    expect(numberFrom('3,5 years')).toBe(3.5);
    expect(numberFrom('no digits here')).toBeNull();
  });

  test('optionsFrom matches on value or on label, case-insensitively', async () => {
