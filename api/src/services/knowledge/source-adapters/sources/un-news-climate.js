'use strict';
/**
 * Per-source adapter — UN News — Climate Change (un-news-climate).
 * Family: rss-feed. Auto-generated from research/un-news-climate.json by
 * scripts/generate-source-adapters.js — edit the research descriptor + regenerate,
 * or add method overrides below the generated block.
 */

const { RssFeedAdapter } = require('../family/rss-feed.adapter');

class UnNewsClimateAdapter extends RssFeedAdapter {
  static key          = "un-news-climate";
  static family       = "rss-feed";
  static capabilities = ["search","browseAll","paginate","filter","download"];
  static filterSchema = [
    {
      "type": "dateFrom",
      "param": "_clientDateFrom",
      "label": "From date",
      "clientSide": true
    },
    {
      "type": "dateTo",
      "param": "_clientDateTo",
      "label": "To date",
      "clientSide": true
    },
    {
      "type": "keyword",
      "param": "_clientKeyword",
      "label": "Keyword (title/description)",
      "clientSide": true
    }
  ];
  static downloadMode = "record-page";
  static notes        = "Feed returns only the most recent ~30 items (observed span July back to May); no historical pagination beyond the feed window.";
  static defaultConfig = {
    "adapterKey": "un-news-climate",
    "url": "https://news.un.org/feed/subscribe/en/news/topic/climate-change/feed/rss.xml"
  };
}

module.exports = UnNewsClimateAdapter;
