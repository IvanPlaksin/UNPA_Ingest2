 ...
 *   beginConsolidation() → snapshot edges → create checkpoint (PENDING)
 *   commitConsolidation() → delete + rewire + archive → checkpoint=COMMITTED
 *   rollbackConsolidation() → restore edges from snapshot → checkpoint=ROLLED_BACK
 */
...
class SubgraphAdapter {
   ...
   */
  async beginConsolidation(subgraphId, namespace, clusterNodeIds) {
    ...
    try {
      ...
      const internalEdges = edgeRes.records.map(rec => ({
  ...
  async commitConsolidation(tx) {
    ...
    try {
      ...
      console.error(`[SubgraphAdapter] Commit failed, rolling back: ${error.message}`);
  ...
  /**
   * Rollback a consolidation transaction — restore edges + unarchive nodes.
   */
  async rollbackConsolidation(tx) {
    const session = this._session();
    ...
    try {
      ...
      for (const edge of tx.internalEdges) {
        try {
          ...
          console.warn(`[SubgraphAdapter] Failed to restore edge ${edge.sourceId}->${edge.targetId}: ${e.message}`);
        }
      }
   ...
   */
  async rollbackFromCheckpoint(checkpointId) {
    const session = this._session();
   ...
   */
  async listCheckpoints(namespace) {
    const session = this._session();
    try {
    ...
    try {
      ...
      for (const rec of res.records) {
        ...
        try {
          ...
          console.error(`[SubgraphAdapter] Recovery failed for ${cpId}: ${e.message}`);
        }
      }
  ...
  async _getCheckpointProp(session, cpId, prop) {
    const res = await session.run(`
      MATCH (cp:ConsolidationCheckpoint {id: $cpId})
...
module.exports = { SubgraphAdapter };
