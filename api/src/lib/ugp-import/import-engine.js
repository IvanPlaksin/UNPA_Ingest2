...
/**
 * ImportEngine (server-side copy of tools/unpa-import/src/lib/import-engine.js).
 * Writes the annotated plan into the target Memgraph in atomic, per-batch managed
 * transactions (`session.executeWrite`, which retries transient conflicts). MERGE
 * key = (primaryLabel, identityProp); MERGE makes every write idempotent, so a
 * re-run after a partial failure converges without duplication. Vectors are
 * imported AFTER the graph (Canonical Write Order) so a mid-transfer failure never
 * leaves dangling edges or orphan vectors.
 */
