...
/**
 * Shared import planner (server-side copy of tools/unpa-import/src/lib/planner.js).
 * Computes the diff between a UGP package and the target Memgraph (+ optionally
 * Qdrant) WITHOUT writing. Identity is per-label (primaryLabel, identityProp).
 * Nodes/relationships are annotated in place with `_action` so the importer can
 * act without re-matching. Kept byte-compatible with the CLI planner.
 */
