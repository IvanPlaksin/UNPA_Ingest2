 ...
 * Document: A/77/489 – ICT Strategy 2023-2028 (UN General Assembly)
 */
...
const PDF_PATH = path.resolve(__dirname, '../../Artefacts/Documents/UN_IT_Regulations/Secretariat/A_77_489_ICT_Strategy_2023-2028.pdf');
const STAGES   = [];
...
async function pollExtraction(docId, maxWaitSec = 600, intervalSec = 10) {
  const deadline = Date.now() + maxWaitSec * 1000;
  while (Date.now() < deadline) {
    try {
...
async function main() {
  console.log('\n' + '='.repeat(65));
  console.log('FULL PIPELINE TEST — A/77/489 ICT Strategy 2023-2028');
  ...
  const uploadResult = await uploadFile(PDF_PATH, {
    namespace:     'UN',
    documentTitle: 'Report of the Secretary-General: ICT Strategy 2023-2028',
    unSymbol:      'A/77/489',
  ...
  log('CLASSIFY', classified.status === 'CLASSIFIED' ? 'OK' : classified.status,
  ...
  log('EXTRACT_PROGRESS', 'monitoring (up to 10 min for large doc)...');
  const progress = await pollExtraction(docId, 600, 10);
  console.log();
  if (!progress) {
  ...
  const [docRow] = await mg.runQuery(
    `MATCH (d:Document {id: $id})
     RETURN d.status AS status, d.documentType AS dtype, d.epistemicLayer AS layer,
            d.extractedAt AS extractedAt, d.kqsScore AS kqs`,
    { id: docId }
  );
  const [erRow] = await mg.runQuery(
    `MATCH (d:Document {id: $id})-[:HAS_EXTRACTION_RESULT]->(r:ExtractionResult)
     RETURN r.id AS id, r.methodologyId AS methId, r.entitiesExtracted AS ents,
            r.vectorsIndexed AS vecs, r.extractionJobId AS jobId, r.completedAt AS ts
  ...
  console.log('='.repeat(65));
  console.log(`Document:   A/77/489 ICT Strategy 2023-2028`);
