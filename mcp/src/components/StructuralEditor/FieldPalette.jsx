  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: 'grab', fontSize: 11, color: '#e2e8f0',
    ...
    >
      <span style={{
        ...
        fontSize: 10, fontWeight: 700, color: ft.color, flexShrink: 0,
      }}>
        {ft.icon}
      </span>
      <span>{ft.label}</span>
...
export default function FieldPalette() {
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '10px 12px', borderBottom: '1px solid #30363d', fontSize: 10, fontWeight: 600, color: '#8b949e', textTransform: 'uppercase', letterSpacing: 1 }}>
        Field Types
      </div>
      <div style={{ flex: 1, overflow: 'auto', padding: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <div style={{ padding: '8px 4px 4px', fontSize: 9, fontWeight: 600, color: '#8b5cf6', textTransform: 'uppercase', letterSpacing: 1, borderTop: '1px solid #30363d', marginTop: 4 }}>
          DataSource
        </div>
        {FIELD_TYPES.filter(ft => ft.dataType?.startsWith('datasource-')).map((ft, i) => (
