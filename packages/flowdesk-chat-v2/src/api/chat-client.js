...
export async function getDraft(sessionId) {
  let res;
  try {
    res = await getFetch()(url(`/flowdesk/draft/${encodeURIComponent(sessionId)}`), {
 ...
 * session). Returns [] when there is none — a non-voice session just yields empty.
 */
export async function getVoiceTranscript(sessionId) {
  let res;
  try {
    res = await getFetch()(url(`/flowdesk/voice/transcript/${encodeURIComponent(sessionId)}`), {
      headers: await buildHeaders(),
    });
  } catch (e) {
    throw new ChatError('NETWORK', e.message);
...
export async function getSchema(serviceId) {
  let res;
  try {
    res = await getFetch()(url(`/flowdesk/schema/${encodeURIComponent(serviceId)}`), {
      headers: await buildHeaders(),
    });
  } catch (e) {
    throw new ChatError('NETWORK', e.message);
 ...
 * `buildHeaders()` with no argument contributes only the host's auth headers,
...
export async function uploadFile(sessionId, file, { signal } = {}) {
  ...
  let res;
  try {
    res = await getFetch()(url(`/flowdesk/chat/upload?sessionId=${encodeURIComponent(sessionId)}`), {
      method: 'POST', body: form, headers: await buildHeaders(), signal,
    });
  } catch (e) {
    throw new ChatError('NETWORK', e.message);
  }
 ...
 *
 * Called from the host's submit callback, once the wizard reports the ticket it
 * made. Safe to call more than once: the server skips anything already linked,
 * so a retry or a double-fire cannot put the same document on a request twice.
 *
...
export async function linkAttachments(sessionId, ticketId, { signal } = {}) {
  let res;
  try {
    res = await getFetch()(url(`/flowdesk/chat/attachments/link?sessionId=${encodeURIComponent(sessionId)}`), {
      method: 'POST',
      headers: await buildHeaders({ 'Content-Type': 'application/json' }),
 ...
 *
 * The one the host actually needs, and the reason it exists separately from
 * `linkAttachments`: a hand-off is terminal. The moment `onOpenForm` fires the
 * component resets to a fresh session with an empty attachment list, so by the
 * time the wizard has produced a ticket the chat no longer knows which
 * conversation staged the files. This takes the very object the host was handed
 * and reads the conversation out of it, so the caller never has to hold a
 * sessionId of its own or guess which one was current.
 *
 * Call it from wherever the wizard reports its result:
 *
 *   onOpenForm={(openForm) => openWizard(openForm, async ({ ticketId }) => {
 *     await linkStagedAttachments(openForm, ticketId);
 *   })}
 *
 * Safe to call always — with nothing staged it returns without a request — and
 * safe to call twice: the server skips what it has already linked.
 *
 * @param {{sessionId?:string, stagedAttachments?:Array}} openForm the hand-off payload
 * @param {string|number} ticketId the ticket the wizard created
 * @returns {Promise<{linked:number, skipped:number, failed:number, details:Array}|null>}
 */
export async function linkStagedAttachments(openForm, ticketId, opts = {}) {
  const staged = (openForm && openForm.stagedAttachments) || [];
  if (!staged.length) return null;

  // The sessionId is on the payload; older payloads only carried it per entry,
  // so fall back to that rather than failing on a stale hand-off.
 ...
 */
export async function sendMessage(sessionId, userId, message, { signal, choice, controlAction, anchor, formEvent, lang, userContext } = {}) {
  ...
  let res;
  try {
    res = await getFetch()(url('/flowdesk/chat'), {
      method: 'POST',
      headers: await buildHeaders({ 'Content-Type': 'application/json' }),
 ...
 *
 * EventSource cannot carry custom headers, so an auth-injecting host can only
 * authenticate this stream via cookies (withCredentials) or the session id in
 * the path. Progress is cosmetic — losing it never loses data — so a rejected
 * stream degrades to a plain "Thinking…" instead of failing the turn.
 *
...
export function subscribeProgress(sessionId, handlers = {}, opts = {}) {
  const ES = opts.EventSourceImpl
    || getConfig().eventSourceImpl
  ...
  let streamUrl;
  try {
    streamUrl = url(`/flowdesk/chat/${encodeURIComponent(sessionId)}/stream`);
  } catch (e) {
    // apiBaseUrl not configured — the POST will surface the real error.
    handlers.onError?.(new ChatError('SSE_DISCONNECT', e.message));
    return () => {};
  }
  let es = null;
  let retries = 0;
  ...
  const wire = () => {
    es = new ES(streamUrl, { withCredentials: true });
    es.onopen = () => { retries = 0; };
